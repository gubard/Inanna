    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        var provider = context
            .SyntaxProvider.CreateSyntaxProvider(
                static (node, _) =>
                    node is AttributeSyntax attribute && attribute.Name.ToString() == "ViewPair",
        ...
        context.RegisterSourceOutput(
            ...
            (spc, pair) =>
            {
                ...
                stringBuilder.AppendLine();
                stringBuilder.AppendLine(
                    $"public static class {compilation.AssemblyName}ViewLocator"
                );
                stringBuilder.AppendLine("{");
                stringBuilder.AppendLine(
                    $"    public static readonly global::{TypeNames.FrozenDictionary}<global::{TypeNames.Type}, global::{TypeNames.Func}<global::{TypeNames.Control}>> Builders;"
                );
                stringBuilder.AppendLine();
                stringBuilder.AppendLine($"    static {compilation.AssemblyName}ViewLocator()");
                stringBuilder.AppendLine("    {");
                stringBuilder.AppendLine(
                    $"        var dictionary = new global::{TypeNames.Dictionary}<global::{TypeNames.Type}, global::{TypeNames.Func}<global::{TypeNames.Control}>>"
                );
                stringBuilder.AppendLine("        {");
                ...
                foreach (var source in list)
                {
                    ...
                    stringBuilder.AppendLine(
                        $"            {{ typeof(global::{viewModelType.GetFullName(compilation)}) , () => new global::{viewType.GetFullName(compilation)}() }},"
                    );
                }
